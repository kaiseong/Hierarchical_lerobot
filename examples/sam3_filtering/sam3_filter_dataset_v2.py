#!/usr/bin/env python3
"""Phase-0 SAM3 filtering test for LeRobot datasets with per-camera feature reuse.
...
Compared with sam3_filter_dataset.py, this v2 path computes SAM3 image features
once per camera frame when role_workers=1, then reuses those features across
the roles for that camera.
...
class TimingAccumulator:
    def __init__(self) -> None:
        ...
        self.image_feature_ms: list[float] = []
        self.image_feature_ms_by_camera: dict[str, list[float]] = defaultdict(list)
    ...
    def add_camera_log(self, cam_log: dict[str, Any]) -> None:
        ...
        feature_elapsed = cam_log.get("image_feature_ms")
        if isinstance(feature_elapsed, (int, float)):
            feature_elapsed_f = float(feature_elapsed)
            self.image_feature_ms.append(feature_elapsed_f)
            self.image_feature_ms_by_camera[camera].append(feature_elapsed_f)
    ...
    def to_summary(self, camera_workers: int, role_workers: int) -> dict[str, Any]:
        return {
            ...
            "image_feature_ms": summarize_ms(self.image_feature_ms),
            "image_feature_ms_by_camera": {
                camera: summarize_ms(values) for camera, values in sorted(self.image_feature_ms_by_camera.items())
            },
...
class MockSegmenter:
    def __init__(self, mock_text_mask: str = "center", confidence_threshold: float = 0.5) -> None:
        ...
        self.last_image_feature_ms: float | None = 0.0

    def segment_roles(self, image: np.ndarray, roles: list[dict[str, Any]]) -> list[RoleResult]:
        self.last_image_feature_ms = 0.0
        return [self.segment_role(image, role) for role in roles]
...
class SAM3ImageSegmenter:
    def __init__(
        ...
        self.confidence_threshold = confidence_threshold
        self.last_image_feature_ms: float | None = None

    @staticmethod
    def _clone_image_state(base_state: dict[str, Any]) -> dict[str, Any]:
    ...
    def _segment_role_from_image_state(
        self,
        ...
        try:
            for prompt in prompts:
                state = self._clone_image_state(base_state)
                if prompt:
    ...
    def segment_role(self, image: np.ndarray, role: dict[str, Any]) -> RoleResult:
        ...
        base_state = self.processor.set_image(pil)
        self.last_image_feature_ms = (time.perf_counter() - feature_start) * 1000
        return self._segment_role_from_image_state(base_state, (h, w), role)
    ...
    def segment_roles(self, image: np.ndarray, roles: list[dict[str, Any]]) -> list[RoleResult]:
        ...
        base_state = self.processor.set_image(pil)
        self.last_image_feature_ms = (time.perf_counter() - feature_start) * 1000
        return [self._segment_role_from_image_state(base_state, (h, w), role) for role in roles]
...
def process_camera(
    ...
    image_feature_ms: float | None = None
    ...
    if role_workers > 1:
        ...
        maybe_feature_ms = getattr(segmenter, "last_image_feature_ms", None)
        if isinstance(maybe_feature_ms, (int, float)):
            image_feature_ms = float(maybe_feature_ms)
    ...
    return {
        ...
        "image_feature_ms": round(image_feature_ms, 3) if image_feature_ms is not None else None,
...
def run(args: argparse.Namespace) -> None:
    ...
    if role_workers > 1:
        print(
            ...
            "Per-camera backbone reuse is disabled when role_workers > 1."
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of roles to process in parallel per frame. Values >1 can multiply "
            "SAM3 model VRAM use and disable v2 per-camera backbone reuse."
        ),
    )
